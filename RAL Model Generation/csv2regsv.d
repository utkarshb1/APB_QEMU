...
class Block
{
  ...
  void print() {
    ...
    writeln("\n\nclass reg_", _name, " extends uvm_reg_block;");
    foreach(reg; _regs) {
      reg.printDeclare();
    }
    writeln("\n");
    ...
    writeln("\n    // define default map");
    writeln("    default_map = create_map(\"default_map\", 32'h0, 4, UVM_LITTLE_ENDIAN, 1);");
    foreach(reg; _regs) {
      reg.printMap();
    }
    writeln("  endfunction: build");
    writeln("\n  `uvm_object_utils(reg_", _name, ")");
    writeln();
    writeln("endclass: reg_", _name);
  }
}
...
class Register
{
  ...
  void printClass() {
    writeln("\n\nclass reg_", _name, " extends uvm_reg;");
    foreach(field; _fields) {
      field.printDeclare();
    }
    writeln("\n  function new(string name = \"", _name, "\");");
    writeln("    super.new(name, ", _n_bits, ", UVM_NO_COVERAGE);");
    writeln("  endfunction: new");
    writeln("\n  virtual function void build();");
    foreach(field; _fields) {
      field.printCreate();
    }
  ...
  void printCreate() {
    writeln("    ", _name, " = reg_",  _name, "::type_id::create(\"",
	    _name, "\");");
  }
  void printConfigure() {
  ...
  void printMap() {
    writefln("    default_map.add_reg(%s, 32'h%x, \"%s\");",
...
class BitField
{
  ...
  void printConfigure() {
    writeln("    ", _name, ".configure(this, ", _size, ", ",
	    _pos, ", \"", _access.toString, "\", ", cast(int) _volatile,
	    ", ", procReset(), ", ", cast(int) _has_reset, ", 0, 0);");
  }
  // TBD
   string procReset() {
     if (_reset_str == "") {
